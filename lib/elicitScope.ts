...
export function createElicitScope() {
  return {
    ...
    // the edge of an auto-sized note. It lives in the authoring kit: it can only
    // ever appear inside a `{ fn }` channel, so it is never a spec keyword.
    noteBox: elicit.authoring.noteBox,
    d3,
    vancouver,
  };
}
