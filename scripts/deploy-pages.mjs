...
if (!pagesRemote.includes(PAGES_REMOTE)) {
    die(`${pagesRoot} points at ${pagesRemote}, not ${PAGES_REMOTE}. Refusing to sync.`);
}

const docsDirty = sh('git', ['status', '--porcelain'], docsRoot);
if (docsDirty && !allowDirty) {
    die(`The docs tree has uncommitted changes, so this build could not be traced\n` +
...
console.log(`  docs   ${sourceBranch} @ ${sourceSha.slice(0, 8)}${docsDirty ? ' (dirty)' : ''}`);
...
const pageCount = sh('find', [outDir, '-name', 'index.html']).split('\n').filter(Boolean).length;
...
execFileSync('rsync', [
    '-a', '--delete',
    '--exclude', '.git/',
    '--exclude', 'CNAME',        // a custom domain, if one is ever configured
    '--exclude', 'README.md',    // the build repo's own explanation of itself
    `${outDir}/`, `${pagesRoot}/`,
], { stdio: 'inherit' });
...
fs.writeFileSync(path.join(pagesRoot, 'README.md'),
    `# elicitjs.github.io\n\n` +
...
// repeat.
function pushPages() {
    if (sh('git', ['config', '--local', '--get', 'http.postBuffer'], pagesRoot) === '') {
        sh('git', ['config', '--local', 'http.postBuffer', '524288000'], pagesRoot);
        sh('git', ['config', '--local', 'http.version', 'HTTP/1.1'], pagesRoot);
    }
    execFileSync('git', ['push', 'origin', 'HEAD'], { cwd: pagesRoot, stdio: 'inherit' });
}
...
function unpushedCommits() {
    try {
        const upstream = sh('git', ['rev-list', '--count', '@{u}..HEAD'], pagesRoot);
...
if (!pagesDirty) {
    if (unpushedCommits() > 0 && !dryRun) {
        step('The build is unchanged, but an earlier run never pushed — pushing now');
        pushPages();
...
if (dryRun) {
    ...
    console.log(sh('git', ['status', '--short'], pagesRoot).split('\n').slice(0, 20).join('\n'));
